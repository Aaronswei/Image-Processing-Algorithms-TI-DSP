...
void main(void)
{
	...
	SciaRegs.SCITXBUF=(char)No_Of_Images;				
	while ( SciaRegs.SCICTL2.bit.TXEMPTY == 0); 	//wait for TX -empty
	
	Input_Image();
	

	Gray_Image('B');    //Usng Blue Component
	Negation_Gray('B'); //Negation of Blue Comp
	Gray_Image('R');	
	Negation_Gray('R');
	Gray_Image('G');
	Negation_Gray('G');
	Gray_Image('A');	//A=> Average =(R+G+B)/3
	Negation_Gray('A');
	Gray_Image('Y');	//Y=> Luminance = 0.3R + 0.59G + 0.11B
	Negation_Gray('Y');
	Gray_Image('X');	//X=> maX   max(R,G,B)
	Negation_Gray('X');	
	Gray_Image('N');	//N=> miN	min(R,G,B)
	Negation_Gray('N');
} 	
